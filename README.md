...
- **Evacuation report**
  ...
  3. `ReportService` builds one sheet per list with status checkboxes (☑/☐ symbol cells for Google Sheets), entrance time, a `Manually updated` `yes`/`no` column, and embedded photos; the controller streams it as XLSX.
