...
- **Cafeteria attendance**: `AttendanceReportService` defines meal time windows, counts unique list item detections per meal (grouping by the list ID from each detection instead of querying per list), adds an "Off the list" row for detections without list items, and passes pivot rows to `ReportService` for XLSX export. A nightly schedule can auto-run the report.
